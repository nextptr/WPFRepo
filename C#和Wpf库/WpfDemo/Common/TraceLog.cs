...
namespace Common
{
    ...
    public class TraceLog
    {
        ...
        public static bool CanDebugTrace = true;

        private TraceLog()
        {
            string logPath = AppDomain.CurrentDomain.BaseDirectory + "\\TraceLogs\\";
        ...
        public static void Debug(string source, object message)
        {
            if (CanDebugTrace)
            {
                Instance.Log(message, MessageType.Debug, source);
            }
        }
        public static void ProcessTace(string source, object message)
        {
