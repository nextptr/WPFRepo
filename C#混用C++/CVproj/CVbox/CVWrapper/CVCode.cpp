...
void CVTest8()//颜色空间缩减 图像元素的遍历和计算
{
...
void color2gray(const uchar* colsrc, uchar* singledst, int w, int h)
{
	Mat matcolor = Mat(h, w, CV_8UC4, (uchar*)colsrc);
	/*Mat matcolor(Size2d(w, h), CV_8UC4);
	memcpy(matcolor.data, colsrc, w * h * 4);*/

	Mat mpdst = Mat(h, w, CV_8UC1, singledst);
	cvtColor(matcolor, mpdst, COLOR_RGB2GRAY);
}
void CVCode::ToolColorConvert(const Mat_CV& src, Mat_CV& dst)
{
	dst.img_h = src.img_h;
	dst.img_w = src.img_w;
	dst.data = new uchar[dst.img_h * src.img_w];
	//color2gray(src.data, dst.data, dst.img_w, dst.img_h);
	int srcType = CVHelper::ConvertType(src.img_type);
	int dstType = CVHelper::ConvertType(dst.img_type);
	Mat matcolor = Mat(src.img_h, src.img_w, srcType, src.data);
	Mat mpdst = Mat(src.img_h, src.img_w, dstType, dst.data);
	cvtColor(matcolor, mpdst, COLOR_RGB2GRAY);
}
